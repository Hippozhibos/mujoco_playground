...
class MS_Jacobian:
    ...
    def apply_constraints(self, J: jp.ndarray) -> jp.ndarray:
        ...
        # 计算正值和负值
        J_pos = jp.maximum(J, 0)  # 取正值部分
        J_neg = jp.minimum(J, 0)  # 取负值部分

        # 计算每一行正值部分的和，并归一化（如果超过 1）
        J_pos_sum = jp.sum(J_pos, axis=1, keepdims=True)
        scale_pos = jp.where(J_pos_sum > 1, 1.0 / (J_pos_sum + 1e-6), 1.0)  # 限制最大为 1
        J_pos = J_pos * scale_pos

        # 计算每一行负值部分的和，并归一化（如果小于 -1）
        J_neg_sum = jp.sum(J_neg, axis=1, keepdims=True)
        scale_neg = jp.where(J_neg_sum < -1, -1.0 / (J_neg_sum - 1e-6), 1.0)  # 限制最小为 -1
        J_neg = J_neg * scale_neg

        # 重新组合 J，并确保每一行的总和为 0
        J = J_pos + J_neg
        row_sums = jp.sum(J, axis=1, keepdims=True)
        J = J - row_sums / J.shape[1]  # 让每行总和归零
